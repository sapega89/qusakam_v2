...
func test_combat_with_coin_spending():
	...
	# Preconditions
	assert_eq(xp_manager.current_level, 1, "Precondition: starts at level 1")
	assert_eq(xp_manager.current_xp, 0, "Precondition: starts with 0 XP")
	assert_eq(xp_manager.xp_for_next_level, 100, "Precondition: level 1 threshold is 100")

	# Kill 5 enemies
	for i in 5:
		xp_manager.add_xp(25)       # 125 XP total so far
		inventory_manager.add_coins(3) # 15 coins total so far
	await _tick(2)

	assert_eq(inventory_manager.get_coins(), 15, "Should have 15 coins after 5 kills")

	# Spend some coins
	var spent = inventory_manager.spend_coins(10)
	await _tick(1)
	assert_true(spent, "Should successfully spend coins")
	assert_eq(inventory_manager.get_coins(), 5, "Should have 5 coins after spending 10")

	# Continue progression (3 more kills -> +75 XP, +9 coins)
	for i in 3:
		xp_manager.add_xp(25)       # 200 XP total gained
		inventory_manager.add_coins(3)
	await _tick(2)
	...
	# Progression checks for linear scaling (req = level * 100)
	assert_eq(xp_manager.current_level, 2, "200 XP should raise level from 1 to 2")
	assert_eq(xp_manager.current_xp, 100, "Remainder should be 100 (200 - 100)")
	assert_eq(xp_manager.xp_for_next_level, 200, "Next threshold should be 200 at level 2")

	# Invariants (extra safety)
	assert_true(xp_manager.current_xp >= 0, "XP must not be negative")
	assert_true(xp_manager.current_xp < xp_manager.xp_for_next_level,
		"XP must be below next threshold after processing level-ups")
