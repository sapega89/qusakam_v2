...
static func get_scatter_documentation() -> DocumentationInfo:
	...
	info.set_title("Scatter2D")
	info.add_paragraph(
		"Scatter2D is a content positioning add-on. It is suited to place
	...
	info.add_paragraph(
		...
		[b]Modifier stack[/b] that's on the [b]Scatter2D[/b] node.")
	info.add_paragraph(
	...
	p.set_type("bool")
	p.set_description(
		"When enabled, Scatter2D will use MultiMeshInstance2D nodes
...
static func get_cache_documentation() -> DocumentationInfo:
	...
	info.add_paragraph(
		...
		[p]+ Press the 'Rebuild' button to scan for other Scatter2D nodes
...
static func get_modifiers_documentation() -> DocumentationInfo:
	...
	p.set_description(
		...
		Scatter2D node will be used instead.")
	...
	p.set_description(
		...
		[p]+ [b]Local[/b]: Modifier operates in local space, relative to the Scatter2D node.[/p]
