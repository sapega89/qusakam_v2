...
func is_point_inside(point: Vector2, global_transform: Transform2D) -> bool:
	...
	if thickness > 0:
		var closest_point_on_curve: Vector2 = curve.get_closest_point(point)
		var dist2 = closest_point_on_curve.distance_squared_to(point)
		if dist2 < _half_thickness_squared:
			return true

	if closed:
		return _polygon.is_point_inside(point)
...
func create_point(position: Vector2) -> void:
	if not curve:
		curve = Curve2D.new()
...
func get_closed_edges(shape_t: Transform2D) -> Array[PackedVector2Array]:
	...
	if closed:
		...
		if first_point != last_point:
			...
			polyline.push_back(first_point)
...
func get_open_edges(shape_t: Transform2D) -> Array[Curve2D]:
...
func _update_polygon_from_curve() -> void:
	...
	for i in baked_points.size():
		...
		var projected_point = Vector2(point.x, point.y)
		_bounds.feed(point)
