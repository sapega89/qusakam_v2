...
@export var auto_rebuild := true:
	set(val):
		auto_rebuild = val
		if not is_instance_valid(_source_node) or not _source_node is Scatter2D:
			return
...
var _source_node: Scatter2D:
	set(val):
		# Disconnect signals from previous scatter node if any
		if is_instance_valid(_source_node) and _source_node is Scatter2D:
...
func _init() -> void:
	...
	documentation.add_paragraph("Copy a modifier stack from another Scatter2D node in the scene.")
...
func _process_transforms(transforms, domain, _seed) -> void:
	...
	if not _source_node or not _source_node is Scatter2D:
		warning += "You need to select a valid Scatter2D node."
		return
